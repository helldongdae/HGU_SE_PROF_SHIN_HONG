...
class htmlHandler implements Visitable{
 ...
 void init(String outputname, int flag){
 if(flag == 1){
 try{
...
\n";
 ...
 String writeFancyHtml(int flag, String token, String sentence){
 // Starts with something
 if(flag == 1){
 if(token.startsWith("h")){
 sentence = sentence.trim();
...
class initialize implements Visitor{
 ...
 handler.init("output.html", 1);
 else
 handler.init("output.html", 2);
 return;
 }
...
class fancyStyle implements Visitor{
 FileInputStream mdFile;
 String line;
 String token;
 fancyStyle(FileInputStream f){
 mdFile = f;
 }
 public FileInputStream visit(getCmd getcmd){
 return null;
 ...
 // Starting condition checker
 if(token.startsWith("h") || token.equals("ul") || token.equals("ol") || token.equals("bq")){
 line = handler.writeFancyHtml(1, token, line);
 }
 else{
 handler.decide();
 handler.hasHeader = "";
 ...
 // Check through the line
 while(true){
 line = handler.fancyAnalyze(line);
 if(line.equals(""))
 break;
 }
 handler.endOfLine();
...
public class fancyStyle {
 ...
 else if(init.option.equals("fancy")){
 fancyStyle mainP = new fancyStyle(mdFile);
 mainP.visit(new getCmd());
 String s = "INITIAL";
 while(true){
 s = mainP.visit(p);
